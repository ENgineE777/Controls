...
void TaskExecutor::GroupTaskPool::FillList()
{
	...
	for (int i = 0; i < (int)taskPools.size(); i++)
	{
		SingleTaskPool* taskPool = taskPools[i];

		for (int j = 0; j < (int)taskPool->lists.size(); j++)
		{
			SingleTaskPool::TaskList* taskList = taskPool->lists[j];

			GroupList* groupList = NULL;

			for (int l = 0; l < (int)groupLists.size(); l++)
			{
...
{
	for (int j = 0; j < (int)groupList.taskLists.size(); j++)
	{
...
void TaskExecutor::GroupTaskPool::Execute(float dt)
{
	...
	for (int i = 0; i < (int)groupLists.size(); i++)
	{
